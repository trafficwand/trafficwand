...
- [x] create `LICENSE` with the standard MIT text, copyright "2026 Ildar Karymov"
- [x] add `BuildInfo.xcconfig` to `.gitignore` (file doesn't exist yet but will be generated in Task 3)
- [x] no automated tests (static files); manual sanity: `head -1 LICENSE` shows `MIT License`
...
- [x] write failing tests in `BuildInfoTests` covering: full dict (version + build + commit), missing commit key (→ `nil`), empty-string commit (→ `nil`), `"unknown"` sentinel commit (→ `nil`), missing version (→ empty string default), missing build (→ empty string default)
- [x] implement `BuildInfo` struct + `init(infoDictionary:)` + `current` static; **no** `versionLine` / `commitLine` helpers (view formats inline)
- [x] run `task test` — tests pass before Task 3
...
- [x] add `build-info` task to `Taskfile.yml` that writes `BuildInfo.xcconfig` containing `GIT_COMMIT = <short hash or "unknown">`; resolve hash via `git rev-parse --short HEAD`, defaulting to `unknown` when not in a work tree or git isn't on PATH
- [x] add `build-info` as a dependency of `build`, `run`, `test`, and the default task in `Taskfile.yml` (`run` inherits via its `build` dep)
- [x] add `xcconfig: BuildInfo.xcconfig` to the `TrafficWand` target in `project.yml` (used XcodeGen's `configFiles` key — the supported syntax for per-config xcconfig wiring)
- [x] add `<key>GitCommitHash</key><string>$(GIT_COMMIT)</string>` to `App/Resources/Info.plist`
- [x] run `task generate` to regenerate the Xcode project from the updated `project.yml`
- [x] run `task build` — verified the built `.app`'s `Info.plist` contains the substituted hash `2da0995` via `defaults read .../TrafficWand.app/Contents/Info GitCommitHash`
- [x] no automated test for this step — `BuildInfo`'s parsing contract is already covered by Task 2; the runtime smoke test isn't viable (test bundle's `Bundle.main` is the xctest runner, not the app), so this is covered by manual verification in Task 8
...
- [x] write failing test asserting `id == rawValue` for each case (sanity check of the `Identifiable` conformance; the enum is trivial otherwise)
- [x] implement `enum SettingsTab: String, CaseIterable, Identifiable { case general, rules, about; var id: String { rawValue } }`
- [x] run `task test` — tests pass before Task 5
...
- [x] create `AboutSettingsView` SwiftUI view rendering: app icon (`NSImage(named: NSImage.applicationIconName)` → `Image(nsImage:)`, ~96pt), app name (from `CFBundleName`), `"Version \(info.version) (build \(info.build))"`, commit line (selectable monospaced text, omitted when `info.commit == nil`), copyright string, license link (opens `Links.license` via `NSWorkspace.shared.open`), Sponsor button (opens `Links.sponsor`)
- [x] declare URL constants and copyright string as a `private enum Links` inside the view file (no separate module)
- [x] inject `BuildInfo` (defaulting to `.current`) and a `(URL) -> Void` open-closure (defaulting to `{ NSWorkspace.shared.open($0) }`) so the view is preview/test-friendly
- [x] add a one-line top-of-file comment noting that visual rendering is covered by manual verification (Task 8), since the repo has no snapshot framework
- [x] no automated test (pure inputs `BuildInfo` already tested in Task 2; URL constants are trivially their own test)
...
- [x] change `SettingsRootView` to take an optional `initialTab: SettingsTab` (default `.general`) and an `@State private var selection: SettingsTab` initialized from it; bind `selection` to the `TabView`; add the `AboutSettingsView` as the third tab with system image `info.circle` and tag `.about`
- [x] add `private(set) var lastRequestedTab: SettingsTab?` to `SettingsWindowController`
- [x] add `func show(initialTab: SettingsTab? = nil)` to `SettingsWindowController`; when non-nil, store in `lastRequestedTab` and reassign the hosting controller's `rootView` to `SettingsRootView(initialTab:)` before activating + ordering front; when nil, leave the rootView untouched (preserves today's behavior)
- [x] write `SettingsWindowControllerAboutTests` asserting: `show(initialTab: .about)` sets `lastRequestedTab` to `.about`; `show(initialTab: .rules)` then `show()` leaves `lastRequestedTab` at `.rules` (the no-arg call must not clear it)
- [x] run `task test` — tests pass before Task 7
...
- [x] add `onOpenAbout: () -> Void = {}` init parameter to `StatusBarController`; insert an `NSMenuItem(title: "About TrafficWand…", action: #selector(openAbout), keyEquivalent: "")` above the existing `Settings…` item, with a separator preserving the existing visual grouping; add `@objc private func openAbout()` that calls `onOpenAbout()`
- [x] add `internal var menu: NSMenu { statusItem.menu! }` as the test seam (replaces no current exposure)
- [x] wire `onOpenAbout: { [weak self] in self?.openAbout() }` in `AppMain.applicationDidFinishLaunching`; add a private `openAbout()` that calls `settingsWindowController?.show(initialTab: .about)`
- [x] write `StatusBarControllerAboutTests` covering: finding the "About TrafficWand…" item by title in `controller.menu`, calling `controller.perform(item.action!, with: item)`, and asserting the injected `onOpenAbout` ran and `onOpenSettings` did not; mirror with a Settings-item case (action invokes `onOpenSettings`, not `onOpenAbout`)
- [x] run `task test` — tests pass before Task 8

### Task 8: Verify acceptance criteria

- [x] verify all items from issue #4 are present in the About tab: app name, app icon, version + build, commit hash, copyright, license notice, sponsor button [manual test (skipped - not automatable); covered by `AboutSettingsView` implementation in Task 5]
- [x] verify status-bar menu item opens Settings deep-linked to About [manual test (skipped - not automatable); wiring verified by `StatusBarControllerAboutTests` + `SettingsWindowControllerAboutTests`]
- [x] verify the License link opens the GitHub LICENSE URL (note: on a dev box where TrafficWand is the default browser and no rule matches `github.com`, the configured fallback fires — picker / specific browser / last-used. This is expected behavior, not a bug.) [manual test (skipped - not automatable)]
- [x] verify the Sponsor button opens `https://github.com/sponsors/tomakado` [manual test (skipped - not automatable)]
- [x] verify Settings → manually switch to General / Rules tabs still works (no regression in tab selection) [manual test (skipped - not automatable)]
- [x] verify `Settings…` from the menu (not "About TrafficWand…") still opens to whatever tab was last shown / `.general` on first open — does NOT pin to `.about` [manual test (skipped - not automatable); the `show(initialTab: nil)` no-op-on-rootView path is verified by `SettingsWindowControllerAboutTests`]
- [x] confirm built `.app`'s Info.plist has a real commit hash via the `defaults read` command from Task 3 — confirmed `GitCommitHash = 01660f5` matches `git rev-parse --short HEAD`
- [x] run `task lint` — clean
- [x] run `task test-core` — green (126 tests passed)
- [x] run `task test` — green (99 tests passed)
- [x] run `task` (default: generate + build + lint + test-core + test) — green
...
- [x] add a short paragraph to `CLAUDE.md` describing the build-info pattern (Taskfile generates `BuildInfo.xcconfig` → `$(GIT_COMMIT)` substituted into `Info.plist` before signing). This is easy to get wrong (PlistBuddy post-build mutation is the common-but-broken alternative) and worth documenting once.
- [x] decide whether `README.md` needs a mention; skip if it doesn't read as user-facing-feature-worthy. — skipped: the About tab is a standard app surface (icon + version + sponsor); not a routing capability, doesn't change the user's workflow, doesn't belong in the feature list.
- [x] `mkdir -p docs/plans/completed` and move this plan file there.
