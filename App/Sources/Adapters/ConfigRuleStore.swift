...
protocol RulePersisting {
    /// Persists a rule that routes future links for `url`'s domain to
    /// `destination` (a concrete `.browser(target)` for a browser/profile pick, or
    /// an `.alias(id)` for an alias pick — the reusable, late-binding rule).
    ///
...
///
/// `remember(url:destination:)` builds the rule with
/// `RememberRule.rule(forURL:destination:)` (scoping the pattern to the registrable
/// domain, e.g. `*x.com`), then loads the current config, computes
/// `AppConfig.upserting(_:)`, and saves the result.
...
struct ConfigRuleStore: RulePersisting {
    ...
    func remember(url: URL, destination: RoutingDestination) {
        guard let rule = RememberRule.rule(forURL: url, destination: destination) else {
