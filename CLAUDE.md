...
- **`TrafficWandCore`** — a local SPM package of **pure Swift (Foundation only, NO
  AppKit)**. All decision-shaped logic lives here: models (`Rule` — whose
  `destination: RoutingDestination` is a concrete `.browser(BrowserTarget)` *or* an
  `.alias(UUID)` reference, `BrowserTarget`, `ProfileAlias` — a named, reusable binding to
  a concrete browser/profile (`id: UUID`, `name`, `target: BrowserTarget`),
  `RoutingDestination`, `FallbackPolicy` — whose `.defaultBrowser` likewise carries a
  `RoutingDestination`, `AppConfig` — the root persisted doc carrying `aliases:
  [ProfileAlias]` plus `AppConfig.upserting` for inserting-or-updating a rule by pattern,
  `Browser`, `BrowserProfile`, `RoutingDecision`), glob matching
...
- **`App/`** — a **thin AppKit/SwiftUI adapter layer**. It owns the menu-bar agent
  ...
  `RoutingService`), Settings (SwiftUI hosted via `NSHostingController`),
  the picker
  ...
  target).
  - **`LinkIntake` makes cold-start intake drop-proof.** `LinkIntake` is a concrete,
    directly-tested `@MainActor` type (`LinkIntakeTests`, not a protocol seam) that buffers
    links arriving via `application(_:open:)` before the routing pipeline is built
    (cold-start safety, issue #16) and flushes them in arrival order as the final step of
    `applicationDidFinishLaunching`. When the pipeline is already wired (warm start) it
    routes immediately — equivalent to the prior direct path.
...
directly; it holds a `RoutingDestination`, which is either a `.browser(BrowserTarget)` or
an `.alias(UUID)` that points at a `ProfileAlias` in `AppConfig.aliases`. Aliases are
late-binding indirection — re-pointing one alias updates every rule that references it.
`RoutingDestination.resolved(in: aliases)` collapses a destination to a concrete
`BrowserTarget?` (`nil` for a **dangling** `.alias` whose id is missing). `Router.decide`
resolves the matched rule's (or the fallback's) destination this way; **a dangling
reference resolves to `.prompt` (the picker)** — never a dropped or misrouted link.
Resolution lives entirely in Core, so `RoutingDecision` and the App launch path
(`RoutingService`) are unchanged — aliases are invisible past the decision boundary.
`RoutingDestination` (and `FallbackPolicy`) use a hand-rolled tagged Codable with a
`type` discriminator.

The persisted `config.json` is at **`AppConfig.currentSchemaVersion = 2`**, and decode
stays backward-compatible: a legacy v1 rule's bare `target` key wraps to
`.browser(...)`, `FallbackPolicy.defaultBrowser`'s legacy `target` likewise wraps, and an
absent `aliases` key defaults to `[]`. New writes always emit the v2 shape. When changing
any persisted model, preserve this migration path (and pin it with a legacy-decode test)
rather than renaming coding keys.
